...
def run_a_trail(train_config, log_file=None, save_mode=False, save_file=None, need_train=True, warm_or_cold=None):
    ...
    data_dir = "dataset/amazon_sports/"
...
if __name__ == '__main__':
    # lr_ = [1e-1,1e-2,1e-3]
    lr_ = [1e-1,1e-2,1e-3]  # [1e-3]  # 1e-2
    dw_ = [1e-2, 1e-4, 1e-6]  # [1e-2, 1e-4, 1e-6]
    # embedding_size_ = [32, 64, 128, 156, 512]
    embedding_size_ = [256]
    ...
    for lr in lr_:
        for wd in dw_:
            for embedding_size in embedding_size_:
                ...
                print(train_config)
                # save_path = "/data0/liuyuting/CoLLM/pretrained/mf/book_lr0.001_wd1e-06_best.pth"
                # save_path = "/data/zyang/LLM/PretrainedModels/mf/0912_ml1m_oodv2_best_model_d256lr-0.001wd0.0001.pth"
                save_path = "pretrained/mf/sports_lr{0}_wd{1}.pth".format(lr, wd)
                f = open("pretrained/mf/sports_lr{0}_wd{1}.log".format(lr, wd), 'w+')
