    ...
    def evaluation(self, model, data_loaders, cuda_enabled=True):
        ...
        for data_loader in data_loaders.loaders:
            ...
            for samples in metric_logger.log_every(data_loader, print_freq, header):
                ...
                # results.extend(eval_output)
                metric_logger.update(loss=eval_output['loss'].item())
                if 'perplexity' in eval_output.keys:
                    metric_logger.update(perplexity=eval_output['perplexity'].item())
                # torch.cuda.empty_cache()
            ...
            if is_dist_avail_and_initialized():
                ...
                uauc, _, _ = uAUC_me(users_.cpu().numpy(), results_logits_.cpu().numpy(), labels_.cpu().numpy())
            ...
            logging.info(
                "Averaged stats: " + str(metric_logger.global_avg()) + " ***auc: " + str(auc) + " ***uauc:" + str(uauc))
